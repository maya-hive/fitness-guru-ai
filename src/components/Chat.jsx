...
export default function Chat() {
    ...
    // Initialize sessionId from localStorage and load initial chat (client-side only)
    useEffect(() => {
        // Only run in browser
        ...
        const initializeChat = async () => {
            ...
            const storedSessionId = localStorage.getItem("fitness_session") || "";
            const currentSessionId = storedSessionId || undefined;

            setLoading(true);
            setError(null);

            try {
                const data = await sendChat({ sessionId: currentSessionId });
    ...
    const planLink = typeof window !== 'undefined' ? `${window.location.origin}/plan/${sessionId}` : '';
